...
Процедура ПроверитьСодержимоеПисьма(Почта, НовыеИдентификаторы, ТемаПисьма, ТекстПисьма)
...
Процедура ОжидатьИПроверитьПисьмо(Почта, Профиль, ПротоколПриема, СтарыеИдентификаторы, ТемаПисьма, ТекстПисьма, ОчищатьУдаленные)
	...
	Для НомерПопытки = 1 По КоличествоПопыток Цикл
		...
		Почта.Подключиться(Профиль, ПротоколПриема);
		...
		ПроверитьСодержимоеПисьма(Почта, НовыеИдентификаторы, ТемаПисьма, ТекстПисьма);
...
Процедура Проверка_ОтправкаПрием(
	Знач ИспользоватьSMTP,
	Знач ИспользоватьPOP3,
	Знач ИспользоватьIMAP,
	Знач ПротоколПриема,
	Знач ПротоколОтправки,
	Знач ПаузаПослеОтправки = 0,
	Знач ОчищатьУдаленные = Ложь)

	Профиль = СоздатьПрофиль(ИспользоватьSMTP, ИспользоватьPOP3, ИспользоватьIMAP);
	...
	Почта.Послать(Сообщение, , ПротоколОтправки);

	Если ПаузаПослеОтправки > 0 Тогда
		Приостановить(ПаузаПослеОтправки); // Подождём, чтобы увидеть письмо в почтовике
	КонецЕсли;

	ОжидатьИПроверитьПисьмо(Почта, Профиль, ПротоколПриема, СтарыеИдентификаторы, ТемаПисьма, ТекстПисьма, ОчищатьУдаленные);
...
Процедура Проверка_ОтправкаПрием_SMTP_POP3()
	...
	Проверка_ОтправкаПрием(Истина, Истина, Ложь,
		ПротоколИнтернетПочты.POP3, ПротоколИнтернетПочты.SMTP);

КонецПроцедуры

Процедура Проверка_ОтправкаПрием_IMAP_IMAP()

	Проверка_ОтправкаПрием(Ложь, Ложь, Истина,
		ПротоколИнтернетПочты.IMAP, ПротоколИнтернетПочты.IMAP, 2000, Истина);

КонецПроцедуры

Процедура Проверка_ОтправкаПрием_SMTP_IMAP()

	Проверка_ОтправкаПрием(Истина, Ложь, Истина,
		ПротоколИнтернетПочты.IMAP, ПротоколИнтернетПочты.SMTP, 2000, Истина);

КонецПроцедуры

Проверка_ОтправкаПрием_SMTP_POP3();
Проверка_ОтправкаПрием_SMTP_IMAP();
