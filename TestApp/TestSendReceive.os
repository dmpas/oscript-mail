...
Процедура ПолучитьСписокПисем()
	...
	Заголовки = Почта.ПолучитьЗаголовки(Новый Структура("Flagged", Истина));

	Для Каждого мСообщение Из Заголовки Цикл
		Сообщить("Заголовок: " + мСообщение.Заголовок);
	КонецЦикла;
	...
	Для Каждого мПисьмо Из Письма Цикл
		...
		Сообщить("---------------------");
		Для Каждого мИД Из мПисьмо.Идентификатор Цикл
			Сообщить(мИД);
		КонецЦикла;

	КонецЦикла;

	Пока Письма.Количество() > 1 Цикл
		Письма.Удалить(0);
	КонецЦикла;

	Почта.УдалитьСообщения(Письма);
	Сообщить("Письмо удалено");
